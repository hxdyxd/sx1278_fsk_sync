...
void SX1276FskInit_Debug( void )
{
    ...
    printf("RegPacketConfig2: %02x \r\n", SX1276->RegPacketConfig2);
    printf("RegDioMapping1: %02x \r\n", SX1276->RegDioMapping1);
    printf("RegOokPeak: %02x \r\n", SX1276->RegOokPeak);
...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_SYNC:
        ...
        if( (read_single_reg(REG_IRQFLAGS1) & RF_IRQFLAGS1_PREAMBLEDETECT ) &&
            ( PreambleDetected == false ) )
        {
        ...
        if( (read_single_reg(REG_IRQFLAGS1) & RF_IRQFLAGS1_SYNCADDRESSMATCH ) &&
            ( PreambleDetected == true ) && ( SyncWordDetected == false ) )
        {
    ...
    case RF_STATE_RX_RUNNING:
...
#if 1
        //if( DIO1 == 1 ) // FifoLevel
        if( read_single_reg(REG_IRQFLAGS2) & RF_IRQFLAGS2_FIFOLEVEL )
        {
        ...
        if( read_single_reg(REG_IRQFLAGS2) & (RF_IRQFLAGS2_PAYLOADREADY | RF_IRQFLAGS2_CRCOK) )
        {
    ...
    case RF_STATE_TX_READY_WAIT:
        //if( DIO3 == 1 )    // TxReady
        if( read_single_reg(REG_IRQFLAGS1) & RF_IRQFLAGS1_TXREADY )
        {
            if( ( SX1276->RegPacketConfig1 & RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE ) == RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE )
            {
    ...
    case RF_STATE_TX_RUNNING:
        ...
        if( read_single_reg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT )
        {
            TxTimeoutTimer = GET_TICK_COUNT( );
